...
fn test_crit(build_stats: HashMap<BaseStat, i64>, attack_lines: Vec<DamageLine>, crit_attack_lines: Vec<DamageLine>, crit_chance: i64, expected_damage: (i64, i64, i64)) {
    let mut build = Build::new_with_stats(Stats::from_map_stats(build_stats.iter()));
    ...
    attack.set_damage_calculation(DamageCalculation::Min);
    build.stats.reset_brutality(&attack);
    assert_eq!(build.evaluate_build_damage(&attack), expected_damage.0);
    attack.set_damage_calculation(DamageCalculation::Average);
    build.stats.reset_brutality(&attack);
    assert_eq!(build.evaluate_build_damage(&attack), expected_damage.1);
    attack.set_damage_calculation(DamageCalculation::Max);
    build.stats.reset_brutality(&attack);
    assert_eq!(build.evaluate_build_damage(&attack), expected_damage.2);
